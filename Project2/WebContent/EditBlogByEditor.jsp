...
<%@page import="elbiscms.DeleteEditBlogByEditor"%>
<%@page import="elbiscms.DeleteEdit"%>
...
try{
	...
	Query1 = "Select blog_title, blog_description, fromDate, toDate, subsection, section_id, website, terminal From Blog Natural Join Section where blog_id = '"
			+ DeleteEditBlogByEditor.blog_id + "' AND section_id = section_id;";
	s1 = sqlConnection.connect().createStatement();
	
	rs1 = s1.executeQuery(Query1);
	while (rs1.next()) {
		blog_title = rs1.getString("blog_title");
		blog_description = rs1.getString("blog_description");
		...
		System.out.println(blog_description);
...
<div id="container">
  <h1>&bull; Beitrag Bearbeiten &bull;</h1>
  <div class="underline">
  </div>
 
  <form action="DeleteEditBlogByEditor" method="post" id="contact_form">
  <%String bl=(String)request.getAttribute("blog");
	DeleteEditBlogByEditor.setData(bl);
   //out.print(bl);
	%>
  <input type="hidden" name="blog" id="blog" /><br>
    <div class="from">
      <label for="from"></label>
      <input type="text" value="<%=fromDate%>" name="from" id="from_input" required>
    </div>
    <div class="to">
      <label for="to"></label>
      <input type="text" value="<%=toDate%>" name="to" id="to_input" required>
    </div>
    <div class="title">
      <label for="title"></label>
      <input type="text" value="<%=blog_title%>" name="title" id="title_input" required>
    </div>
    <div class="subsection">
      <label for="subsection"></label>
      <select name="subsection" id="subsection_input" required>
        ...
        <%
			...
			try{
				...
				while(rs.next())
				{
					if(rs.getInt("section_id")==Integer.valueOf(section_id)){
					%>
    ...
    <div class="content">
      ...
      <textarea name="content" id="content_input" cols="30" rows="5" required><%=blog_description%></textarea>
    </div>
    <center>
    <p>
  ...
  <br>
    <div class="submit">
      ...
      <input type="submit" value="Aktualisieren" id = "update" name = "act" onclick="clickAlert()"/>
